...
bool Octree::bWorldGeometryDirty = false;
...
void Octree::MarkWorldGeometryDirty() {
	bWorldGeometryDirty = true;
}

bool Octree::ConsumeWorldGeometryDirty() {
	const bool bWasDirty = bWorldGeometryDirty;
	bWorldGeometryDirty = false;
	return bWasDirty;
}

void Octree::ClearCacheForCurrentWorld() {
	if (World == nullptr || OctreeMin <= 0.f || OctreeMax <= 0.f) {
		return;
	}
	...
	IFileManager::Get().DeleteDirectory(*filePath, false, true);
	UE_LOG(
		LogHolodeck,
		Log,
		TEXT("Octree: Cleared cache directory %s"),
		*filePath);
}
