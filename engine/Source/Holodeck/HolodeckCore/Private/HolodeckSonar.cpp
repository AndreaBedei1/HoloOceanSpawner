...
void UHolodeckSonar::ResetOctreeState() {
	if (octree != nullptr) {
		delete octree;
		octree = nullptr;
	}

	for (Octree* AgentTree : agents) {
		delete AgentTree;
	}
	agents.Reset();
...
void UHolodeckSonar::TickSensorComponent(
	...
	if (Octree::ConsumeWorldGeometryDirty()) {
		UE_LOG(
			LogHolodeck,
			Log,
			TEXT("HolodeckSonar: world geometry changed, rebuilding octree."));

		Octree::ClearCacheForCurrentWorld();
		ResetOctreeState();
		initOctree();
	}
